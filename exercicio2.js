...
proxImg.addEventListener('click', function() {
  ...
  imgAtual = (imgAtual+1)%7;
  img.setAttribute('src', servidorDasImagens + imagens[imgAtual].arquivo);
  img.setAttribute('alt',  imagens[imgAtual].descricao);
});

antImg.addEventListener('click', function() {
  let img = document.querySelector('#slide');
  imgAtual = (imgAtual+6)%7;
  img.setAttribute('src', servidorDasImagens + imagens[imgAtual].arquivo);
  img.setAttribute('alt',  imagens[imgAtual].descricao);
});
